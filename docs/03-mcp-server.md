...
MCP (Model Context Protocol) is a standard way for a model's host application to ask a server "what can you do?" and then call those things over JSON-RPC 2.0. This server is a thin translator: one gRPC client to the engine, seven tools, three resources, one prompt. The design work is ergonomics for the model: say just enough, in the model's units, with the arithmetic already done, and make every error something the model can fix in one retry.
...
| `ping` | `{}` |
| `tools/list` | The seven tool definitions with input schema, output schema and annotations |
...
| `list_trades` | `limit` 1–50 | compact rows, newest first | read-only |
...
* **The arithmetic is done here.** Quotes, notionals and average prices are computed in integer math so the model never sums seven price levels itself.
