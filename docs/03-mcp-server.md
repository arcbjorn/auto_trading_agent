...
This server is a thin translator: one gRPC client, nine tools, three resources, one prompt. The
...
A tool the model does not need on a turn still costs context: the nine definitions are about a thousand tokens on the Messages API side. The chat service keeps that cost to one cache write per session by never changing the list (see [04](04-agent-service.md)).
