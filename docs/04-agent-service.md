...
All tool results of one assistant turn go back in a single user message, as the API requires for parallel tool use. A tool the model calls outside the turn's permission is answered with an error result (`tool_not_permitted`) and flagged, so a misbehaving model call can never reach the engine.
