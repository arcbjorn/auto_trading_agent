...
* Chat: through `POST /chat`, with every tool call, the gate's decision on it and the verifier's flags next to the reply; the session's state; the audit log's tail with a verify button and a tamper button. Six buttons run the hostile scripted models against the gate over the whole suite.
