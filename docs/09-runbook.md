...
| agent-service | see [04 Agent service](04-agent-service.md) | |
...
ENGINE_ADDR=http://127.0.0.1:50051 python scripts/mcp_interop_check.py --stdio target/debug/mcp-server
cargo run -p mcp-server -- --http &
python scripts/mcp_interop_check.py --http http://127.0.0.1:8000/mcp
```

Both runs end with `INTEROP OK`.
...
| `{"rejected": true, "code": "RATE_LIMIT"}` | More than `POLICY_ACTIONS_PER_MINUTE` actions on one account; wait a minute |
