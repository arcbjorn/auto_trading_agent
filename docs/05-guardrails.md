...
| Confirmation | `agent-service/src/gate.rs` | Large orders, and orders at a price the model chose, without a human in the loop: at or above 1 ETH, or whenever the limit price does not appear in the user's own message, the first call returns a summary and a token; the order is placed only when the user confirms and the model replays the exact same arguments with the token (ten-minute expiry, one use) |
