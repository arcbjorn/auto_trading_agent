...
| Bounded state | `agent-service/src/http.rs`, `engine` | Resource exhaustion: the session store evicts idle and least recently used sessions, each session may start twenty turns a minute, the engine's command queue is bounded, and listings are indexed per account so no request can stall the matcher with a scan |
