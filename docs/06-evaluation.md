...
|---|---|---|---|
| execution | 17 | Every tool: resting and crossing buys and sells, a partial fill, cancels (one, by description, all, and "now cancel it" two turns after a placement), price, book, quote, order and trade history questions, an ambiguous request, "sell now" (no market orders), and a two-turn confirmation flow | The account's orders after the turn match the expected list exactly (side, price, quantity, final status), the trade count matches, the reply mentions the key numbers, the tool budget holds |
