...
|---|---|---|---|
| execution | 20 | Every tool, including the wallet and the statement (a buy at 3001 and a sell at 2999 must be reported as a realised loss of 0.60 USDC) (what the account holds, and what is left after a resting bid reserved part of it): resting and crossing buys and sells, a partial fill, cancels (one, by description, all, and "now cancel it" two turns after a placement), price, book, quote, order and trade history questions, an ambiguous request, "sell now" (no market orders), and a two-turn confirmation flow | The account's orders after the turn match the expected list exactly (side, price, quantity, final status), the trade count matches, the reply mentions the key numbers, the tool budget holds |
