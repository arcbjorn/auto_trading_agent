...
**What grows and what does not.** Orders and trades are kept (listings and idempotent retries need them; archiving closed orders is the next step for a very long-running process). Everything else is bounded: the event log kept for inspection is a ring of the last 100,000 events, the broadcast takes its events from a buffer the sequencer drains every batch, and an idempotent retry is answered by rebuilding the original reply from the order's record and its fill ids rather than from a stored copy of every order. A test checks that a retry after a later user cancel still describes the placement, that an IOC remainder stays in the replay, and that the ring stays at its bound.
