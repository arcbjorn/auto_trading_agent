...
impl ToolSet {
    ...
    pub fn definitions(&self) -> Vec<Value> {
        ...
        vec![
            ...
            json!({
                "name": "list_trades",
                "title": "List trades",
                "description": "This account's executed trades, newest first. Call when the user asks what has filled or for trade history.",
                "inputSchema": { "type": "object", "properties": {
                    "limit": { "type": "integer", "minimum": 1, "maximum": 50, "default": 10 } },
                    "additionalProperties": false },
                "outputSchema": { "type": "object", "properties": { "trades": { "type": "array" }, "count": { "type": "integer" } }, "required": ["trades", "count"] },
                "annotations": read_only
            }),
        ]
    }
    ...
    async fn place(&self, args: &Value) -> ToolOutput {
        ...
        let mid_ticks = match self.market().await {
            Ok(m) if m.best_bid_ticks > 0 && m.best_ask_ticks > 0 => {
                Some((m.best_bid_ticks as u64 + m.best_ask_ticks as u64) / 2)
            }
            Ok(_) => None,
            Err(e) => return e,
        };
        ...
        if let Err(rej) = self.policy.check_place(&self.account, price, qty, mid_ticks, open) {
