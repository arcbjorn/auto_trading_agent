...
Read tools (get_market_summary, get_order_book, get_quote, list_orders, list_trades) are always safe. \
place_limit_order and cancel_order change the account's orders: call them only when the user explicitly asked to trade or cancel, \
