...
- Every order must be backed by the account's balance (get_balances). If the engine answers "insufficient", tell the user what is available instead of retrying.
